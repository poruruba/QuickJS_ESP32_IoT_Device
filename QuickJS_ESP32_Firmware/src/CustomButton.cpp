 ...
 *----------------------------------------------------------------------*/
CustomButton::CustomButton(uint8_t pin, uint8_t invert, uint32_t dbTime) {
    _pin    = pin;
    _invert = invert;
    _dbTime = dbTime;
 ...
 *----------------------------------------------------------------------*/
uint8_t CustomButton::read(void) {
    static uint32_t ms;
 ...
 *----------------------------------------------------------------------*/
uint8_t CustomButton::isPressed(void) {
    return _state == 0 ? 0 : 1;
}

uint8_t CustomButton::isReleased(void) {
    return _state == 0 ? 1 : 0;
}
 ...
 * These functions do not cause the button to be read.                  *
 *----------------------------------------------------------------------*/
uint8_t CustomButton::wasPressed(void) {
    return _state && _changed;
}

uint8_t CustomButton::wasReleased(void) {
    return !_state && _changed && millis() - _pressTime < _hold_time;
}

uint8_t CustomButton::wasReleasefor(uint32_t ms) {
    _hold_time = ms;
 ...
 * These functions do not cause the button to be read.                  *
 *----------------------------------------------------------------------*/
uint8_t CustomButton::pressedFor(uint32_t ms) {
    return (_state == 1 && _time - _lastChange >= ms) ? 1 : 0;
}

uint8_t CustomButton::releasedFor(uint32_t ms) {
 ...
 * in milliseconds.                                                     *
 *----------------------------------------------------------------------*/
uint32_t CustomButton::lastChange(void) {
    return _lastChange;
}